...
## 서비스의 차별점: 클라이언트 소유권 증명 대행
기존의 워터마크 시스템은 서버가 자신의 소유권을 증명하기 위해 워터마크를 삽입하는 방식이 일반적입니다.  
그러나 **Invisible Proof**는 **클라이언트의 소유권을 증명**하는 대행 서비스로 설계되었습니다.  

### 주요 특징
1. **클라이언트 중심의 소유권 증명**  
   서버는 클라이언트가 업로드한 이미지에 대해 클라이언트의 소유권을 증명하는 워터마크를 삽입합니다.  
   이 워터마크는 클라이언트의 소유권을 명확히 나타내며, 서버는 이를 보증하는 역할을 수행합니다.

2. **키 교환 및 서명 검증**  
   클라이언트와 서버 간 RSA 키 교환 및 서명 검증 단계를 포함하여, 워터마크 삽입 과정의 신뢰성을 보장합니다.  
   - 클라이언트는 자신의 RSA 키 쌍을 생성하고, 공개키를 서버에 전달합니다.  
   - 서버는 클라이언트의 공개키를 사용하여 클라이언트의 요청을 검증하고, 서버의 RSA 개인키로 서명된 증거 토큰을 반환합니다.  
   이를 통해 클라이언트와 서버 간의 상호 신뢰를 기반으로 한 워터마크 삽입이 이루어집니다.

3. **제3자 인증 역할**  
   서버는 클라이언트와 제3자 간의 신뢰를 보장하는 중립적인 인증 기관 역할을 수행합니다.  
   클라이언트는 서버가 발급한 증거 토큰과 워터마크를 사용하여 자신의 소유권을 제3자에게 입증할 수 있습니다.
...
### 해결 방안
1. **보이지 않는 워터마크 삽입**  
   `blind_watermark`(https://github.com/guofei9987/blind_watermark) 라이브러리를 사용하여 이미지에 소유권 정보를 삽입합니다. 이 워터마크는 이미지의 시각적 품질에 영향을 주지 않으며, 변조 방지 기능을 제공합니다.

2. **RFC3161 표준 타임스탬프 발급**  
   현재 FreeTSA.org를 통해 RFC3161 표준 타임스탬프를 생성하여 이미지의 생성 시점을 증명합니다.  
   이 타임스탬프는 법적 효력을 가지지는 않지만, 향후 법적 효력을 가진 타임스탬프 발급 기관(TSA)과 연동할 수 있도록 기술적으로 동일한 방식으로 구현되었습니다.
...
- **클라이언트 서명 검증**
  클라이언트는 워터마크 요청 시 자신의 RSA 개인키로 서명한 데이터를 포함하며, 서버는 클라이언트의 공개키를 사용하여 서명을 검증합니다.  
  이를 통해 요청의 무결성과 클라이언트의 신원을 보장합니다.

- **보이지 않는 워터마크 삽입**  
  이미지에 소유권 정보를 삽입하여 변조 방지 및 소유권 증명을 지원합니다.

- **RFC3161 타임스탬프 발급**  
  FreeTSA.org를 통해 생성된 타임스탬프는 법적 효력을 가지지는 않지만, 이미지의 생성 시점을 증명하며 향후 법적 효력을 가진 TSA와 연동할 수 있도록 설계되었습니다.
...
sequenceDiagram
    ...
    Server->>Server: 암호화 패키지 검증(Client 신원 검증)
